...
#Função para saber a diferença entre GM e GS
diferencia = lambda reg: reg[GM] - reg[GS]

#ordenar la lista según los puntos en primer lugar y, si hay empates,
# según la diferencia entre goles marcados y goles sufridos
ordenar = lambda reg: (pontos(reg), (diferencia(reg)))
...
def main():
    ...
    tab = sorted(tabela, key=diferencia,reverse=True)
    printTabela(tab)
    ...
    tab = sorted(tabela, key=ordenar,reverse=True)
    printTabela(tab)
