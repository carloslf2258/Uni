...
def registaCompra(produtos):
    ...
    while code != "":
        ...
        if len(lst) == 1 and code in produtos:  # Se só é introduzido o código, é uma unidade do produto
            ...
            print("{}   {}  {:.2f}".format(produtos[lst[0]][0], quantidade,
                                           # O preço do produto é multiplicado pela quantidade do mesmo
                                           (produtos[lst[0]][2] + produtos[lst[0]][2] * produtos[lst[0]][
                                               3]) * quantidade))
...
def fatura(produtos, compras):
    ...
    produtos_comprados = {key: produtos[key] for key in
                          compra}  # Seleciona os produtos do dicionario cuja chave se encontra na compra
    for key, value in sorted(produtos_comprados.items(), key=lambda e: e[1][
        1]):  # Ordena o dicionario alfabeticamente com base na secção dos produtos
    ...
    for i in produtos_comprados:
        ...
        preco_total = float(
            (produtos_comprados[i][2] + produtos_comprados[i][2] * produtos_comprados[i][3]) * compra[i])
