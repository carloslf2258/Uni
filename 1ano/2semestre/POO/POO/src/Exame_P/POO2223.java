...
public class POO2223 {
	...
	private static void alinea2(PrintStream out) {
		...
		*/
		try (Scanner sc = new Scanner(new FileReader("bookings.csv"))) {
			String line;
			while ((line = sc.nextLine()) != null) {
				String[] bookingDetails = line.split(",");
				String spaceType = bookingDetails[0];
				String name = bookingDetails[1];
				String location = bookingDetails[2];
				int rentDays = Integer.parseInt(bookingDetails[3]);
		
				
				if (spaceType.equals("Caravan")) {
					campingService.registerBooking(new CaravanBooking(name, location, rentDays));
				} else if (spaceType.equals("Car")) {
					campingService.registerBooking(new CarBooking(name, location, rentDays));
				} else if (spaceType.equals("Tent")) {
					campingService.registerBooking(new TentBooking(name, location, rentDays));
				} else {
					System.out.println("Unknown booking type: " + spaceType);
				}
			}
