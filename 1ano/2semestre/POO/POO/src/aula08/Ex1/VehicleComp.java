...
public class VehicleComp {
    ...
    public void printVehicleWithMostKm() {
        ...
        for (Vehicle vehicle : this.vehicles)
            if (vehicle.distanciaTotal() > max.distanciaTotal())
                max = vehicle;
    ...
    private void removeVehicle(Vehicle vehicle) {
        ...
        int index = 0;
        for (Vehicle v : this.vehicles)
            if (!v.getPlaca().equals(vehicle.getPlaca()))
                newVehicles[index++] = v;
        this.vehicles = newVehicles;
    }
    ...
    public String toString() {
        return String.format("Company %s%nNumber of vehicles: %d%nTotal distance: %d", this.name, this.vehicles.length, this.distanciaTotal());
    }
}