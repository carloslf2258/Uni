...
Graph* GraphCopy(const Graph* g) {
  ...
  for (; i < g->numVertices; ListMoveToNext(vertices), i++) {
    ...
    for (; j < ListGetSize(edges); ListMoveToNext(edges), j++) {
      struct _Edge* e = ListGetCurrentItem(edges);
      // Add the weighted edge to the copy graph
...
Graph* GraphFromFile(FILE* f) {
  ...
  // Read the graph properties from the file
  fscanf(f, "%d %d %d %d", &isDigraph, &isWeighted, &numVertices, &numEdges);

  // Create a new graph with the given properties
...
int GraphRemoveEdge(Graph* g, unsigned int v, unsigned int w) {
  ...
  // Move to the vertex v
  ListMove(g->verticesList, v);
  struct _Vertex* vertex = ListGetCurrentItem(g->verticesList);

  // Move to the edge w in the edgesList of vertex v
  ListMove(vertex->edgesList, w);
  struct _Edge* edge = ListGetCurrentItem(vertex->edgesList);

  // Remove the edge from the edgesList and free the memory
  ListRemoveCurrent(vertex->edgesList);
  free(edge);

  // Update the number of edges and outDegree of vertex v
  g->numEdges--;
  vertex->outDegree--;

  // If the graph is undirected, remove the corresponding edge from vertex w to vertex v
  if (g->isDigraph == 0) {
    // Move to the vertex w
    ListMove(g->verticesList, w);
    struct _Vertex* vertex = ListGetCurrentItem(g->verticesList);

    // Move to the edge v in the edgesList of vertex w
    ListMove(vertex->edgesList, v);
    struct _Edge* edge = ListGetCurrentItem(vertex->edgesList);
    ...
    // Update the outDegree of vertex w
    vertex->outDegree--;
  }
...
int GraphCheckInvariants(const Graph* g) {
  ...
  for (; i < g->numVertices; ListMoveToNext(vertices), i++) {
    ...
    // Check if the vertex id matches its position in the list
    if (v->id != i) {
      return 0;
    }

    // Check if the outDegree of the vertex matches the number of edges in its edgesList
    if (v->outDegree != ListGetSize(v->edgesList)) {
      return 0;
    }

    // If the graph is not a digraph, check if the inDegree of the vertex matches the number of edges in its edgesList
    if (g->isDigraph == 0) {
      if (v->inDegree != ListGetSize(v->edgesList)) {
        return 0;
      }
    }
    ...
    // Iterate through each edge in the edgesList of the vertex
    for (; j < ListGetSize(edges); ListMoveToNext(edges), j++) {
      struct _Edge* e = ListGetCurrentItem(edges);

      // Check if the adjacent vertex of the edge is within the valid range
      if (e->adjVertex >= g->numVertices) {
        return 0;
      }

      // If the graph is not weighted, check if the weight of the edge is 1.0
      if (g->isWeighted == 0) {
        if (e->weight != 1.0) {
          return 0;
        }
  ...
  // If the graph is not a digraph, check if the number of edges matches the expected number
  if (g->isDigraph == 0) {
    if (g->numEdges != g->numVertices * (g->numVertices - 1) / 2) {
      return 0;
    }
  }
  // If the graph is a digraph, check if the number of edges matches the expected number
  else {
